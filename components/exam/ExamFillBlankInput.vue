...
const parsedParts = computed<ParsedPart[]>(() => {
  ...
  // 同时匹配 HTML <img> 标签和 Markdown 图片语法
  ...
  while ((match = imgRegex.exec(props.content)) !== null) {
    // match[1] = HTML img src, match[3] = Markdown image path
    const src = match[1] || match[3]
    if (src) {
      allMatches.push({ type: 'image', index: match.index, length: match[0].length, value: src })
    }
  }
